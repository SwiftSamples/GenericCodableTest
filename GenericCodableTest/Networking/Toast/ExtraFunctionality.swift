...
extension UIImageView {
    ...
    func getImage(withUrl : String, placeHolder : UIImage?,imgContentMode : UIView.ContentMode = .scaleAspectFill,imgContentModeOfPlaceHolder : UIView.ContentMode = .scaleAspectFit){
        //        print("ImageURL: \(withUrl)")
...
extension String {
    ...
    func equals(with list: [String])-> Bool{
    ...
    func notEquals(with list: [String])-> Bool{
        for i in 0..<list.count{
            if list[i] == self{
                return false
            }
    ...
    func isValidPassword() -> Bool {
        let passwordCharacters = "^(?=.*[A-Z])(?=.*[!@#$%&*?])(?=.*[0-9])(?=.*[a-z]).{8,}$"
        let testPassword = NSPredicate(format: "SELF MATCHES %@", passwordCharacters)
...
extension String {
    func stringToDate(inputFormat: String) -> Date {
        let dateFormatterGet = DateFormatter()
        dateFormatterGet.dateFormat = inputFormat // "yyyy-MM-dd HH:mm:ss"
        
//        let dateFormatterOut = DateFormatter()
//        dateFormatterOut.dateFormat = "MMM dd,yyyy"
        
//        let dateOut: Date? = dateFormatterGet.date(from: "2017-02-14 17:24:26")
        let dateOut: Date? = dateFormatterGet.date(from: self)
//        print(dateFormatterOut.string(from: dateOut!))
        
//        let dateFormatter = DateFormatter()
//        dateFormatter.dateFormat = inputFormat
//        let date = dateFormatter.date(from: inputFormat) ?? Date()
        return dateOut ?? Date()
    }
...
extension UIViewController {
    ...
    func checkAndPushPop<T : UIViewController>(_ viewCon: T,navigationController : UINavigationController?, shouldAnimate: Bool? = true, completion : ((UIViewController?)->())? = nil) {
        for vc in navigationController?.viewControllers ?? [UIViewController]() {
            print(vc)
            print(viewCon)
            if vc is T {
                navigationController?.popToViewController(vc, animated: shouldAnimate!) {
                    completion?(navigationController?.viewControllers.last)
                }
                return
            }
        }
        navigationController?.pushViewController(viewCon, animated: shouldAnimate!)
    }
    
//    func showDropDown(with : [String]?,from : UIView, direction: DropDown.Direction = .any, completion : @escaping(String,Int)->()){
...
extension UIAlertController {
    ...
    func changeToCustomFont() {
        if let tt = self.title{
            let titles = NSMutableAttributedString(string: tt, attributes: [NSAttributedString.Key.font:UIFont(name: "Cairo-Bold", size: 18) ?? .boldSystemFont(ofSize: 18.0)])
            self.setValue(titles, forKey: "attributedTitle")
        }
        if let mm = self.message{
            let messagess = NSMutableAttributedString(string: mm, attributes: [NSAttributedString.Key.font:UIFont(name: "Cairo-Regular", size: 16) ?? .systemFont(ofSize: 16.0)])
            self.setValue(messagess, forKey: "attributedMessage")
        }
...
extension String {
    ...
    func heightOfString(width : CGFloat, usingFont font: UIFont) -> CGFloat {
...
extension UIViewController {
    ...
    @objc func checkAndProcessToLogin(from : UIViewController?,needToPop : Bool = true,after : (()->())?){
        DispatchQueue.main.async {
//            if CommonUserDefaults.accessInstance.isLogin(){
//                after?()
//            }else{
//                //                let nav = Helper.getVcObject(vcName: .AuthNavigationController, StoryBoardName: .Auth) as! AuthNavigationController
//                //                nav.loginClosure = { isSucces in
//                //                    if isSucces{
//                //                        after?()
//                //                    }else{
//                //                        if needToPop {
...
//                //                    }
//                //                }
//                //                let login = Helper.getVcObject(vcName: .LoginVC, StoryBoardName: .Auth) as! LoginVC
...
//            }
    ...
    func showProgressBarInAlert(_ completion : @escaping (UIAlertController,UIProgressView) -> ()){
        ...
        self.present(alertView, animated: true, completion:{
            ...
            progressView.tintColor = #colorLiteral(red: 0.7450980544, green: 0.1568627506, blue: 0.07450980693, alpha: 1)
    ...
    func alert (title:String){
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.0) {
            ...
            alertController.changeToCustomFont()
...
extension UIScrollView {
    ...
    func addRefreshControl(onRefresh : ((UIRefreshControl?)->())?){
        let refresh = UIRefreshControl()
        refresh.addAction(for: .valueChanged){
...
// MARK: Popover with tableView
class PopoverView: NSObject, UITableViewDelegate, UITableViewDataSource, UIPopoverPresentationControllerDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.attributedText = NSAttributedString(string: self.items?[indexPath.row] ?? "", attributes: [NSAttributedString.Key.foregroundColor: UIColor.black, NSAttributedString.Key.font: UIFont(name: "Roboto-Regular", size: 16) ?? UIFont()])
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        pickerView.dismiss(animated: true, completion: {
            self.delegate?.value(didSelect: self.items?[indexPath.row] ?? "", at: indexPath.row, indexPath: self.indexPath ?? IndexPath(), sender: self.sender ?? UIView())
        })
    }
    ...
    func show(with items: [String], sender: UIView, showDirection: UIPopoverArrowDirection = .up, indexPath: IndexPath = IndexPath(), header: String? = nil) {
        self.items = items
        ...
        pickerView.preferredContentSize = CGSize(width: 150, height: pickerView.tableView.contentSize.height)
        let popOverVC = pickerView.popoverPresentationController
        popOverVC?.backgroundColor = .gray
    ...
    func showFromHeader(with items: [String], sender: UIBarButtonItem, showDirection: UIPopoverArrowDirection = .up, indexPath: IndexPath = IndexPath(), header: String? = nil) {
        self.items = items
        self.indexPath = indexPath
        self.header = header
        pickerView.tableView.delegate = self
        ...
        pickerView.modalPresentationStyle = .popover
        pickerView.preferredContentSize = CGSize(width: 150, height: pickerView.tableView.contentSize.height)
        pickerView.view.backgroundColor = header == nil ? .white : UIColor(red: 229/255, green: 229/255, blue: 229/255, alpha: 1)
        let popOverVC = pickerView.popoverPresentationController
        popOverVC?.containerView?.layer.shadowColor = UIColor.black.cgColor;
        popOverVC?.permittedArrowDirections = showDirection
        popOverVC?.containerView?.layer.shadowOpacity = 0.4;
