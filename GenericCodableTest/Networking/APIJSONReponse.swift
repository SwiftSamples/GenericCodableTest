...
class APIJSONReponse {
    ...
    init(data: Data?, dict: Dictionary<String, AnyObject>?, response: URLResponse?,error: Error?,rpcErrorData: ErrorDataModel?){
        self.data = data
        self.dict = dict
        self.response = response
        self.error = error
        self.rpcErrorData = rpcErrorData
        //If not error
        if (self.error == nil) {
                ...
                if(baseModel.status != APIResponseStatusKeys.success){
                    if let message  = baseModel.message {
                        if self.HTTPResponse.statusCode != 200 {
                            self.error = NSError(domain:APIError.domain, code: self.HTTPResponse.statusCode, userInfo:[NSLocalizedDescriptionKey:message])
                        } else {
    ...
    init(error: Error? ,dataDict : NSDictionary, rpcErrorData: ErrorDataModel? = nil){
        self.data = nil
        self.dict = nil
        self.response = nil
        self.error = error
